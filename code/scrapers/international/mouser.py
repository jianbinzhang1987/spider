...
class MouserScraper(BaseScraper):
    ...
    def login(self, context: BrowserContext, page: Page) -> bool:
        ...
        try:
            username_input = page.locator(
                "input[id*='Username'], input[name*='Username'], input[type='email'], input[type='text']"
            ).first
            ...
            if page.locator(
                ...
                self.wait_for_human_intervention(
                    ...
                    "text=我的账户, text=退出, a[href*='SignOut'], text=My Account",
                )
            ...
            if "login" not in page.url.lower():
                ...
                if page.locator("text=我的账户, text=退出, text=My Account").first.is_visible():
    ...
    def search_and_extract(self, page: Page, model: str, quantity: int) -> Dict[str, Any]:
        ...
        # Use the search endpoint that typically yields better results
        ...
        if not product_links:
            product_links = page.locator(
                ".search-result a[href], .product-links a, table a[href*='mouser']"
            ).all()
    ...
    def _extract_details(self, page: Page, model: str, quantity: int) -> Dict[str, Any]:
        ...
        try:
            qty_input = page.locator(
                "#multqty, #txtQty, input[id*='Quantity'], input[name*='qty'], "
                "input[id*='quantity']"
            ).first
    ...
    def _extract_single_price(self, page: Page):
        for sel in ["#spnPrice", ".price-num", ".pdp-price", "[class*='price']"]:
            try:
